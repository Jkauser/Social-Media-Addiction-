...
if Nv == "Prediction":
    ...
    if st.button("Predict"):
        if op=="Logistic Regression":
            #y_pred_LR= LogReg.predict(xt)    
            st.subheader(f"\nPredictedion by using Logistic Regression is:")
            #st.success(y_pred_LR)
    
        if op=="Random Forest":
            #y_pred_RF= random_forest.predict(xt)    
            st.subheader(f"\nPredictedion by using Random Forest is:")
            #st.success(y_pred_RF)

        
        if op=="Decision Tree":
            #y_pred_DT= DecTree.predict(xt)    
            st.subheader(f"\nPredictedion by using Decision Tree is:")
            #st.success(y_pred_DT)

        if op=="KNN":
            #y_pred_KNN= DecTree.predict(xt)    
            st.subheader(f"\nPredictedion by using KNN is:")
            #st.success(y_pred_KNN)
        
        if op=="Naive Bayes":
            #y_pred_NB= NB.predict(xt)    
            st.subheader(f"\nPredictedion by using Naive Bayes is:")
            #st.success(y_pred_NB)
        
        if op=="SVM":
            #y_pred_SVM= svm.predict(xt)    
            st.subheader(f"\nPredictedion by using SVM is:")
